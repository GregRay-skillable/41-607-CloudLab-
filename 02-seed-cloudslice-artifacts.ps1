...
if (-not $app) {
    throw "App registration '$AppDisplayName' was not found. Run LCA 01 first."
}
...
$seedRoot = Join-Path (Get-CloudSliceWorkRoot) 'seed'
New-Item -ItemType Directory -Path $seedRoot -Force | Out-Null
New-Item -ItemType Directory -Path (Join-Path $seedRoot 'site') -Force | Out-Null
New-Item -ItemType Directory -Path (Join-Path $seedRoot 'repo') -Force | Out-Null
New-Item -ItemType Directory -Path (Join-Path $seedRoot 'business') -Force | Out-Null
...
"@ | Set-Content -Path (Join-Path $seedRoot 'researcher-report.txt') -Encoding UTF8
...
"@ | Set-Content -Path (Join-Path $seedRoot 'repo/appsettings.production.json') -Encoding UTF8
...
"@ | Set-Content -Path (Join-Path $seedRoot 'business/deployment-notes.txt') -Encoding UTF8
...
"@ | Set-Content -Path (Join-Path $seedRoot 'business/customer-queue.csv') -Encoding UTF8
...
'@ | Set-Content -Path (Join-Path $seedRoot 'site/index.html') -Encoding UTF8

Write-Log 'Uploading investigation and business artifacts to blob storage.'

Set-AzStorageBlobContent -Context $storageContext -Container 'investigation-artifacts' -File (Join-Path $seedRoot 'researcher-report.txt') -Blob 'researcher-report.txt' -Force -ErrorAction Stop | Out-Null
Set-AzStorageBlobContent -Context $storageContext -Container 'investigation-artifacts' -File (Join-Path $seedRoot 'repo/appsettings.production.json') -Blob 'leaked-repo/appsettings.production.json' -Force -ErrorAction Stop | Out-Null
Set-AzStorageBlobContent -Context $storageContext -Container 'business-files' -File (Join-Path $seedRoot 'business/deployment-notes.txt') -Blob 'operations/deployment-notes.txt' -Force -ErrorAction Stop | Out-Null
Set-AzStorageBlobContent -Context $storageContext -Container 'business-files' -File (Join-Path $seedRoot 'business/customer-queue.csv') -Blob 'exports/customer-queue.csv' -Force -ErrorAction Stop | Out-Null
Set-AzStorageBlobContent -Context $storageContext -Container 'web-content' -File (Join-Path $seedRoot 'site/index.html') -Blob 'index.html' -Force -ErrorAction Stop | Out-Null
